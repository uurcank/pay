...
class Pay::Shopify::Billable::Test < ActiveSupport::TestCase
  setup do
  ...
  test "doesn't allow fake processor by default" do
    assert_raises Pay::Error do
      users(:none).set_payment_processor :shopify
    end
  end

  test "allows shopify processor if enabled" do
    assert_nothing_raised do
      users(:none).set_payment_processor :shopify, allow_fake: true
    end
  end

  test "shopify processor customer" do
  ...
  test "shopify processor subscribe" do
    assert_difference "Pay::Subscription.count" do
      @pay_customer.subscribe
    end
  end

  test "shopify processor subscribe with promotion code" do
    assert_difference "Pay::Subscription.count" do
      @pay_customer.subscribe(promotion_code: "promo_xxx123")
    end
  end

  test "shopify processor add payment method" do
  ...
  test "generates shopify processor_id" do
    user = users(:none)
    pay_customer = user.set_payment_processor :shopify, allow_fake: true
